                ...
                public void onResponse(Call<ApiResponse<Integer>> call, Response<ApiResponse<Integer>> response) {
                    if (response.isSuccessful()) {
                        ...
                        if ("200".equals(apiResponse.getValue().getStatus())) {
                            ...
                            Call<ApiResponse<PaymentUrlResponse>> callApiDrink = paymentApiService.createPaymentUrl(paymentRequest);

                            callApiDrink.enqueue(new Callback<ApiResponse<PaymentUrlResponse>>() {
                                @Override
                                public void onResponse(Call<ApiResponse<PaymentUrlResponse>> call, Response<ApiResponse<PaymentUrlResponse>> response) {
                                    if (response.isSuccessful()) {
                                        ApiResponse<PaymentUrlResponse> apiResponse = response.body();
                                        if ("200".equals(apiResponse.getValue().getStatus())) {
                                ...
                                @Override
                                public void onFailure(Call<ApiResponse<PaymentUrlResponse>> call, Throwable t) {
                                    Toast.makeText(CartActivity.this, "Network error", Toast.LENGTH_SHORT).show();
                                }
                            });
                        } else {
